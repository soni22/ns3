 ...
 * wireless.cc
 * 
 * Copyright 2014 sonika <sonikayadav822@gmail.com>
 * 
...
main (int argc, char *argv[])
{
  ...
  AnimationInterface::SetNodeColor (wifiApNode, 0, 255, 0); 
  AnimationInterface::SetNodeColor (wifiStaNodes, 255, 0, 0); 
  AnimationInterface::SetNodeColor (csmaNodes, 0, 0, 255); 
  AnimationInterface::SetBoundary (0, 0, 35, 35); 

  AnimationInterface anim ("wireless.xml"); // Mandatory

  anim.EnablePacketMetadata (true); // for metadata 
  anim.EnableIpv4RouteTracking ("routingtable-wireless.xml", Seconds (0), Seconds (5), Seconds (0.25)); // Generating XML file for routing tables
  Simulator::Run ();
  Simulator::Destroy ();
  return 0;
}
