...
//            p2p       p2p
//
// - UDP flows from n1 to n2, then to n3
...
//
  ...
  p2p.EnableAscii(ascii.CreateFileStream ("task-1.tr"), dev);
  p2p.EnablePcap("task-1", dev, false);
...
//
  ...
  Ptr<FlowMonitor> monitor = flowmon.InstallAll();
	...
	AnimationInterface anim ("task-1.xml");
	anim.SetConstantPosition(n.Get(0), 1.0,1.0);
	anim.SetConstantPosition(n.Get(1), 2.0,2.0);
...
//
  ...
  monitor->SerializeToXmlFile("task-1.flowmon", true, true);
